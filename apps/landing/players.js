...
const players = [
  { name:"MS DHONI", country:"India", role:"Wicket Keeper", basePrice:20000000 },
  { name:"ROHIT SHARMA", country:"India", role:"Batter", basePrice:20000000 },
  { name:"VIRAT KOHLI", country:"India", role:"Batter", basePrice:20000000 },
  { name:"R ASHWIN", country:"India", role:"All-Rounder", basePrice:20000000 },
  { name:"TRENT BOULT", country:"New Zealand", role:"Bowler", basePrice:20000000 },
  { name:"PAT CUMMINS", country:"Australia", role:"Bowler", basePrice:20000000 },
  { name:"KAGISO RABADA", country:"South Africa", role:"Bowler", basePrice:20000000 },
  { name:"DAVID WARNER", country:"Australia", role:"Batter", basePrice:20000000 },
  { name:"JASPRIT BUMRAH", country:"India", role:"Bowler", basePrice:20000000 },
  { name:"QUINTON DE KOCK", country:"South Africa", role:"Wicket Keeper", basePrice:20000000 },
  { name:"RAVINDRA JADEJA", country:"India", role:"All-Rounder", basePrice:20000000 },
  { name:"MITCHELL STARC", country:"Australia", role:"Bowler", basePrice:20000000 },
  { name:"SHREYAS IYER", country:"India", role:"Batter", basePrice:20000000 },
  { name:"KANE WILLIAMSON", country:"New Zealand", role:"Batter", basePrice:20000000 },
  { name:"MOHAMMAD SHAMI", country:"India", role:"Bowler", basePrice:20000000 },
...
function renderPlayers(){
    ...
    /* Built as one string and assigned once. `innerHTML +=` inside the loop
       re-parses the entire table on every row, which is visibly slow at 300+
       players. The markup below is kept identical to the rows generated by
       tools/build-players-table.mjs so a cleared search restores the page to
       exactly what it was served as. */
    ...
    tableBody.innerHTML =
        filtered.map(player => `
                <tr>
                    ...
                    <td data-label="Role"><span class="role-badge">${player.role}</span></td>
                    <td class="price" data-label="Base Price">${formatPrice(player.basePrice)}</td>
                </tr>`).join("");
}

/* Deliberately no initial renderPlayers() call: the full, unfiltered list is
   already in players.html, written there by tools/build-players-table.mjs so
   that crawlers and JS-less visitors get the real content. Re-rendering it on
   load would only throw that markup away and rebuild an identical copy.
   renderPlayers() takes over from the first search / filter / sort. */
